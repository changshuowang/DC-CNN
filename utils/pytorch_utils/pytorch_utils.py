...
class DCConv(nn.Module):
    ...
    def forward(self, input): # input: (B, 3 + 3 + C_in, npoint, centroid + nsample)
        ...
        if self.relation_prior == 2:
            abs_coord = input[:, 0:2, :, :]
            delta_x = input[:, 3:5, :, :]
            zero_vec = Variable(torch.zeros(x.size()[0], 1, x.size()[2], K).cuda())
        else:
        ...
        coord_xi = abs_coord[:, :, :, 0:1].repeat(1, 1, 1, K)   # (B, 3, npoint, nsample),  centroid point
        ...
        x = torch.mul(torch.pow(torch.abs((x + 0.000001)), self.p), torch.sign(x))
        
        x = assign_score(score=scores, point_input = x)  # b,n,k,cin,
        x = x.permute(0, 3, 1, 2)  # b,cin,n,k

        x = F.max_pool2d(self.activation(self.bn_dcconv(x)), kernel_size=(1, K)).squeeze(3)  # (B, C_in, npoint)
