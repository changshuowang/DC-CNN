...
class PointnetSAModuleMSG(_PointnetSAModuleBase):
    ...
    nsamples : list of int32
        Number of samples in each ellipsoid query
    ...
    def __init__(
        ...
        for i in range(len(radii)):
            ...
            if use_xyz:
                mlp_spec[0] += 3
            if npoint is not None:
                self.mlps.append(pt_utils.SharedDCConv(mlp_spec, num_kernel = num_kernel, mapping = mapping, relation_prior = relation_prior, first_layer = first_layer))
...
class PointnetSAModule(PointnetSAModuleMSG):
    ...
    radius : float
        Radius of ball
    nsample : int
        Number of samples in the ellipsoid query
    mlp : list
